...
class CameraService {
  ...
  Future<void> startControllerFor(CameraLensDirection dir) async {
    if (_cameras.isEmpty) return;
  ...
  Future<XFile?> _captureRearPhoto(CameraDescription backDesc) async {
    ...
    if (_controller == null ||
      ...
      await startControllerFor(CameraLensDirection.back);
    }
  ...
  Future<Map<String, XFile?>> _retryWithControllerSwitching(
    ...
    try {
      ...
      // Capture avant
      await startControllerFor(CameraLensDirection.front);
      await Future.delayed(const Duration(milliseconds: 300));
      try {
