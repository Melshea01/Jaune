...
class _MyHomePageState extends State<MyHomePage> with TickerProviderStateMixin {
  ...
  Future<void> _loadInitialData() async {
    try {
      await ModelPredictor.loadModels();
      await _loadState();
    } catch (e) {
      debugPrint('Warning: failed to load initial data: $e');
    }
  }
      ...
      builder: (context) => CupertinoAlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            const Text("Chaque fois que tu bois, appuie sur le bouton « 🍻 »"),
            const SizedBox(height: 8),
            const Text("1 verre standard = 1 clic (ex : une pinte = 2 clics)."),
            const SizedBox(height: 8),
            const Text("Ton 🍋 a des points de vie qui montent ou descendent selon ta consommation."),
            const SizedBox(height: 8),
            // Rich text with link functionality could be moved to a separate widget
          ],
        ),
        actions: [
              ...
              Row(
                ...
                children: [
                  CupertinoButton(
                    ...
                    child: Icon(
                      Icons.info_outline,
                      color: Colors.grey.shade100.withAlpha((0.7 * 255).round()),
                      size: 24,
                    ),
                  ),
                ],
                      ...
                      Positioned(
                        ...
                        child: AnimatedBuilder(
                          ...
                          builder: (context, _) {
                            return CustomPaint(
                              size: Size(
                                (MediaQuery.of(context).size.width * 0.50).clamp(0, 260),
                                (MediaQuery.of(context).size.width * 0.22).clamp(0, 70),
                              ),
